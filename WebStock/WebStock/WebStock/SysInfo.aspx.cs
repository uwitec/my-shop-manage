...
namespace WebStock
{
    public partial class SysInfo : System.Web.UI.Page
    {
        ...
        private DataTable GetSystemInfo()
        {
            ...
            //	Server Name
            Assign(table, "机器名称", Server.MachineName);
            Assign(table, "服务器IP", Request.ServerVariables["LOCAl_ADDR"]);
            Assign(table, "WEB服务名称", Request.ServerVariables["Server_Name"]);
            Assign(table, "WEB服务端口", Request.ServerVariables["Server_Port"]);
            //	Web Server
            //Assign(table, "Web Server Version", Request.ServerVariables["Server_SoftWare"]);
            //	Path
            Assign(table, "虚拟目录", Request.FilePath);
            Assign(table, "物理路径", Request.PhysicalPath);
            Assign(table, "虚拟根目录", Request.ApplicationPath);
            Assign(table, "物理根路径", Request.PhysicalApplicationPath);
            //	Platform
            ...
            Assign(table, "服务器当前时间", DateTime.Now.ToString());
        ...
        private void LoadInformation(DataTable table)
        {
            ...
            ASPxRoundPanel1.Controls.Add(div);
        }
